...
function SocialLink({
  ...
  return (
    <Link
      className="group inline-flex h-8 w-8 items-center justify-center rounded-full border border-[var(--mono-border)]/20 bg-[var(--mono-surface)]/60 transition-transform duration-150 hover:-translate-y-0.5"
      {...props}
    >
      <Icon className="h-3.5 w-3.5 fill-[var(--mono-text-muted)] transition-colors duration-150 group-hover:fill-[var(--mono-accent)]" />
    </Link>
  )
}
...
function Role({ role }: { role: Role }) {
  ...
  return (
    <li className="grid grid-cols-[auto_1fr] items-start gap-x-3 gap-y-1 py-3 first:pt-0 last:pb-0">
      <div className="flex h-10 w-10 items-center justify-center rounded-sm border border-[var(--mono-border)]/20 bg-[var(--mono-surface)]">
        <Image
          src={role.logo}
      ...
      <dl className="grid grid-cols-1 gap-y-1 sm:grid-cols-[1fr_auto] sm:items-center">
        <div>
          ...
          <dd className="text-[0.7rem] font-semibold uppercase tracking-[0.18em] text-[var(--mono-text)]">
            {role.company}
          </dd>
          <dt className="sr-only">Role</dt>
          <dd className="text-[0.62rem] uppercase tracking-[0.16em] text-[var(--mono-text-muted)]">
            {role.title}
          </dd>
        </div>
        <div className="sm:text-right">
          ...
          <dd
            className="text-[0.58rem] uppercase tracking-[0.16em] text-[var(--mono-text-muted)]"
...
function Resume() {
  ...
  return (
    <div className="rounded-sm border border-[var(--mono-border)]/20 bg-[var(--mono-surface)]/70 p-6">
      <h2 className="flex items-center gap-3 text-[0.7rem] font-semibold uppercase tracking-[0.14em] text-[var(--mono-text)]">
        <BriefcaseIcon className="h-5 w-5 flex-none" />
        Work Log
      </h2>
      <ol className="mt-4 divide-y divide-[var(--mono-border)]/15">
    ...
    <div className="mx-auto max-w-5xl space-y-16">
      ...
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
        <div className="space-y-6">
          <p className="text-[0.6rem] uppercase tracking-[0.18em] text-[var(--mono-text-muted)]">
            Senior Software Engineer · Founder · Product Builder
          </p>
          <h1 className="text-[2rem] font-medium tracking-tight text-[var(--mono-text)] sm:text-[2.4rem] sm:leading-[1.1]">
      ...
      <div className="grid grid-cols-1 gap-16 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
        <section className="space-y-5">
          <div className="flex flex-col gap-2 border-b border-[var(--mono-border)]/15 pb-3">
            <h2 className="text-[0.7rem] font-semibold uppercase tracking-[0.14em] text-[var(--mono-text)]">
              Latest Articles
            </h2>
          </div>
